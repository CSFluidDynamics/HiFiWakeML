...
class WindFormer(nn.Module):
    def __init__(self, in_channels, out_im_channels, batchnorm=True, dropout=0.0, bc=64):
        super().__init__()
        self.W = torch.nn.Parameter(torch.randn(1,1,510))
        self.W.requires_grad = True
        self.decoder_layer = nn.TransformerDecoderLayer(d_model=bc*8, nhead=8)
        ...
        self.down5 = down(bc*8, bc*8, batchnorm, dropout=dropout)
        self.up0 = up(bc*16, bc*8, batchnorm, dropout=dropout)
    ...
    def forward(self, x, y):
        ...
        x = x6 = self.down5(x5)
        shape = x6.shape
        z = torch.cat([y, self.W.repeat(y.shape[0],y.shape[1],1)],-1)
        x6 = x6.view(shape[0],shape[1],-1).permute(2,0,1)
        x += self.transformer_decoder(x6,z.permute(1,0,2)).permute(1,2,0).view(shape)
        print(x.shape)
        x = self.up0(x, x5)
        x = self.up1(x, x4)
        x = self.up2(x, x3)
...
class UNetp(nn.Module):
    def __init__(self, in_channels, out_im_channels, mlp_in_channels, mlp_out_channels, batchnorm=True, dropout=0.0, bc=64):
        ...
        self.up4 = up(bc*2, bc*2, batchnorm, dropout=dropout)
        self.outc = outconv(bc*2, out_im_channels)
        self.mlp = mlp(mlp_in_channels, mlp_out_channels)
    def forward(self, x, c):
        ...
        x4 = self.down3(x3)
        x = x5 = self.down4(x4)
        c = self.mlp(c, x5.shape[-2],x5.shape[-1])
...
class mlp(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        nn.ReLU(inplace=True),
        nn.Linear(out_ch,out_ch))
    def forward(self, x, dim_1, dim_2):
        x = self.mlp(x).unsqueeze(-1).unsqueeze(-1)
        return x.repeat(1,1,dim_1,dim_2)
